...
<head>
  ...
  <title>AP CSP Reflection – Aryan Shrimali</title>
  <style>
    body {
...
<body>
  ...
  <p><strong>Tools</strong> → Tools setup for me was initially a nightmare, a domain I was far from familiar with. Concepts such as using <code>Windows Subsystem for Linux (WSL)</code>, utilizing terminal commands to navigate directories, and understanding how GitHub repositories worked were far from my past experiences. With the help of Mr. Mortenson and the multitude of avenues he provided for one to immerse oneself in tool-setting processes, my peers and I slowly but surely understood the means to set up our tools, ready to explore the realm of computer science.</p>
  ...
  <p>
    I would love to delve into concepts such as <strong>Backend Development</strong> and how to construct <strong>APIs</strong> (Application Programming Interfaces). Learning these terms from individuals in our class with previous coding experience, such as Evan and Lillian, I became intrigued as to what they represent and how programmers utilize them through the everyday applications we use.
  </p>
  ...
  <p>
    ...
    28) B → D (8 bits = 256 characters)<br />
    29) D → A (The AND gate will produce an output of false, and the first OR gate will produce an output of false; therefore, both outputs = FALSE)<br />
    37) D → C (Segment I w/ more arithmetic vs. segment II)<br />
    40) A → D (All three determined by total points calculation per student, before vs. after points difference, and midterm/final score comparison)<br />
    42) C → D (IPv4 has 2³² possible addresses. IPv6 has 2¹²⁸ possible addresses. IPv6 ≈ 2×10⁹⁶ more addresses)<br />
    45) C → A (Expression NOT (P AND Q) evaluates to false when both P and Q are true)<br />
    46) C → D (An undecidable problem is one in which no algorithm can always lead to a correct yes-or-no answer)<br />
    48) C → D (Experiment successful when RANDOM(1–100) ≤ 75 → 75% of the time)<br />
    50) C → B (List does not need to be sorted because the procedure checks list elements until the target is found or list ends)<br />
    54) C → D (Procedures <code>square</code> and <code>cube</code> each determine a power of n)<br />
    56) C → D (Starts at 1 and increases count by 2 → counts only odd integers)<br />
    64) B&D → B&C (Cloud computing with security considerations for users)
  </p>
